...
function generateStatistics() {
  return {
    statistics: [
      { id: 'stat-001', title: 'Total Tasks', icon: '📊', value: 156, change: '+12', changeLabel: 'this week', changeType: 'positive', color: '#1976D2' },
      { id: 'stat-002', title: 'Completed', icon: '✅', value: 89, change: '+8', changeLabel: 'today', changeType: 'positive', color: '#388E3C' },
      { id: 'stat-003', title: 'In Progress', icon: '🔄', value: 42, change: '0', changeLabel: 'Same as yesterday', changeType: 'neutral', color: '#FF6F00' },
      { id: 'stat-004', title: 'Overdue', icon: '⚠️', value: 25, change: '+3', changeLabel: 'today', changeType: 'negative', color: '#D32F2F' }
    ],
...
function writeJsonFile(filename, data) {
  // write into project's src/assets/mocks by default
  const filePath = path.join(__dirname, '..', 'src', 'assets', 'mocks', filename);
...
function main() {
  ...
  try {
    ...
    // Also write a combined db.json for json-server (use arrays for resources)
    const db = {
      tasks: tasks.tasks,
      statistics: statistics.statistics,
      users: users.users
    };
    writeJsonFile('db.json', db);
